...
1. **공고문 원문 확보** — `gov_subsidy/sample_data/`에 실제 정부지원사업 사업운영지침 PDF(샘플)를
   넣어둡니다. 웹 검색 요약만으로는 지급 회차 구조나 신청기한 계산식 같은 세부 규정이 부정확해질
   수 있어서, 원문 PDF를 직접 파싱해 대조하는 것을 기본 원칙으로 합니다.
2. **조건 파싱** — `subsidy_rule_parser.py`가 PDF에서 지원대상 요건(연령, 근로시간, 급여 상한),
   지급 회차·금액, 신청기한 계산 규정(예: "근속 도달일이 속한 달의 익월부터 2개월 이내")을
   구조화된 JSON 규칙으로 추출합니다.
3. **케이스 대입 계산** — `subsidy_schedule_calculator.py`에 채용일·근무지(수도권/비수도권,
   지역 등급)를 입력하면, 회차별 근속 도달일 → 신청 가능일 → 신청 마감일 → 지급 예상 시기를
   자동 계산해 표로 출력합니다.
4. **결과 저장** — `python gov_subsidy/subsidy_schedule_calculator.py --hire-date 2026-04-01 --region 비수도권`
