biz-support-toolkit
경영지원팀(회계·재무 / 인사·총무 / 구매·계약 / 법무) 실무 자동화를 위한 파이썬 스크립트 모음 모든 스크립트는 뼈대가 아니라 가상의 샘플 데이터가 함께 들어 있어 clone 후 명령어 한 줄이면 실제 결과물을 바로 확인할 수 있도록 세팅했습니다.

// 모든 데이터는 가상의 값으로 회사별 데이터 구조에 맞춰 커스텀 필요.카테고리 & 스크립트
카테고리	스크립트	설명	실행 명령
...
gov_subsidy	`subsidy_schedule_calculator.py`	정부지원사업 공고문(PDF) 원문을 직접 읽어 채용 케이스별 지급 회차·신청기한 자동 산출	`python gov_subsidy/subsidy_schedule_calculator.py`
...
과거 재직자 이력서 학습 — `hr/sample_data/past_hires_resumes.csv`에는 "장기재직(긍정패턴)"
그룹뿐 아니라 "조기퇴사(위험패턴, 6개월 이내 퇴사)" 그룹도 함께 들어 있습니다. 장기재직자만
보면 "이미 뽑힌 사람들은 원래 비슷하다"는 생존편향에 빠지기 쉬워서, 조기퇴사 사례의 공통점도
대비군으로 함께 학습합니다.
8개 범용 핏 카테고리로 분류 — `hr/fit_taxonomy.py`에 정의된 전문성_실행력, 커뮤니케이션,
...
학습 결과 저장 — `python hr/build_fit_profile.py`를 실행하면 `hr/output/company_fit_profile.json`
으로 저장됩니다. (`.env`는 API 키 같은 설정값 전용이고, 실제로 "학습된" 데이터는 이 JSON
파일입니다.)
신규 지원자 리포트 생성 — `python hr/resume_fit_report.py`를 실행하면 지원자 이력서와
...
> 결과 하나로 내리면 안 됩니다.
정부지원사업 관리 A to Z: 공고문 대조 기반 지급일정 자동화
`gov_subsidy/` 폴더는 경영지원팀이 반복적으로 하는 "정부지원사업 조건 확인 → 우리 회사 케이스 대입 →
지급일정 계산" 작업을 자동화한 파이프라인입니다.
공고문 원문 확보 — `gov_subsidy/sample_data/`에 실제 정부지원사업 사업운영지침 PDF(샘플)를
넣어둡니다. 웹 검색 요약만으로는 지급 회차 구조나 신청기한 계산식 같은 세부 규정이 부정확해질
수 있어서, 원문 PDF를 직접 파싱해 대조하는 것을 기본 원칙으로 합니다.
조건 파싱 — `subsidy_rule_parser.py`가 PDF에서 지원대상 요건(연령, 근로시간, 급여 상한),
지급 회차·금액, 신청기한 계산 규정(예: "근속 도달일이 속한 달의 익월부터 2개월 이내")을
구조화된 JSON 규칙으로 추출합니다.
케이스 대입 계산 — `subsidy_schedule_calculator.py`에 채용일·근무지(수도권/비수도권,
지역 등급)를 입력하면, 회차별 근속 도달일 → 신청 가능일 → 신청 마감일 → 지급 예상 시기를
자동 계산해 표로 출력합니다.
결과 저장 — `python gov_subsidy/subsidy_schedule_calculator.py --hire-date 2026-04-01 --region 비수도권`
처럼 실행하면 `gov_subsidy/output/`에 케이스별 일정표(csv/pdf)가 생성됩니다.
> ⚠️ 이 도구는 규정 원문 파싱 결과를 참고용으로 정리해주는 것이며, 최종 신청·수령 여부는
> 반드시 고용24(work24.go.kr) 및 해당 연도 공고문으로 재확인해야 합니다. 지침 개정 시
> `sample_data/`의 원문 PDF를 최신본으로 교체해야 계산 결과가 최신 규정을 반영합니다.
>
> 다음 단계(로드맵): 회사 채용 시트(입사일·소속·급여)와 연동해 대상자 전원의 신청 일정을
> 자동 산출하고, 신청 마감 D-7 알림을 보내는 기능은 아직 구현 전입니다.
실행 결과 미리보기
finance
...
python gov_subsidy/subsidy_schedule_calculator.py
```
